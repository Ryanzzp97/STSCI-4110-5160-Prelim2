...
lrfunction <- function(i, j, data) {
  ...
  result <- data.frame(term1 = i, term2 = j, pvalue = pvalue, significance = paste0(round(pvalue, 4),significance))
  return(result)
}
...
```{r}
# Evaluate Full Model
full.model <- glm(event ~.-diabetes+educ*gender, data = data, family = binomial)
summary(full.model)
...
probs <- predict(reg, type = "response")
