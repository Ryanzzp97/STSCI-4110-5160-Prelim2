...
```{r}
# Load the CSV file
data_old <- read.csv("/Applications/Lincy's Folder/Career & School/Cornell Fall 2023/ILRST 4110/STSCI-4110-5160-Prelim2/Handout/final_cardiac_data.csv")
...
# choice<-c("gender","ethnic1","educ","diabetes","smoker")
for (i in 1:(choose(length(choice),2))){
interaction.plot(
  x.factor = as.factor(data[[combn(choice,2)[,i][1]]]),
  trace.factor = as.factor(data[[combn(choice,2)[,i][2]]]),
  response = as.numeric(data$event), type = "l", legend = TRUE,
  xlab=combn(choice,2)[,i][1],
  ylab="Event",
  trace.label = combn(choice,2)[,i][2]
)
  
  
# interchange xlab and ylab:
interaction.plot(
  x.factor = as.factor(data[[combn(choice,2)[,i][2]]]),
  trace.factor = as.factor(data[[combn(choice,2)[,i][1]]]),
  response = as.numeric(data$event), type = "l", legend = TRUE,
  xlab=combn(choice,2)[,i][2],
  ylab="Event",
  trace.label = combn(choice,2)[,i][1]
)
...
## previously excluded data cleaning
data$bmi <- cut(data$bmi, breaks=c(-Inf, 18.5, 24.9, 29.9, Inf), labels=c("Underweight","Normal","Overweight", "Obese"))
